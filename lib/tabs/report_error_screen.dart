...
class _ReportErrorScreenState extends State<ReportErrorScreen> {
  ...
  void _enviaEmail(msg, BuildContext context) {
    ...
    String bodyEmail = '<h3>Usuário: ${UserModel.of(context).userData['name']}</h3>'+
                '<h3>Email: ${UserModel.of(context).userData['email']} </h3>'
                '<p>Mensagem: $msg</p>';
    String sender = UserModel.of(context).userData["email"];
    String subject = 'Contato Usuário';

    // Email it.
    new Email().sendEmail(bodyEmail, sender, "murilo08inter@gmail.com", subject).then((envelope) {
      resultEmail =
